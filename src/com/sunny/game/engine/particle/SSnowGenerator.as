package com.sunny.game.engine.particle
{
	 ...
	 *
	 */
	public class SSnowGenerator extends SBasicParticleGenerator
	{
		public var snowRadius : int;
		public var speed : Number;
		public var density : Number;

		public function SSnowGenerator(parent : Sprite, viewRect : Rectangle, density : Number = 20, radius : int = 5, speed : Number = 300, startTick : int = 150)
		{
			super(parent, viewRect);

			this.snowRadius = radius;
			this.speed = speed;
		...
		{
			super.update();
			var d : Number = density / 60 * elapsedTimes;
			var l : int = int(d);
			...
			for (var i : int = children.length - 1; i >= 0; i--)
			{
				...
				if (child)
				{
					...
					{
						child.y += elapsedTimes / 1000 * child.speed * Math.sin(child.r);
						child.x += elapsedTimes / 1000 * child.speed * Math.cos(child.r);
					}
				}
			}
		}
		...
		{
			return new Snow(snowRadius, speed);
		}
