package com.sunny.game.engine.utils.easing
{
	import com.sunny.game.engine.events.STickEvent;
	import com.sunny.game.engine.transition.tween.easing.SEaseLinear;
	import com.sunny.game.engine.utils.SMathUtil;
	...
	[Event(name="tween_start", type="com.sunny.game.engine.utils.easing.TweenEvent")]
	[Event(name="tween_end", type="com.sunny.game.engine.utils.easing.TweenEvent")]
	[Event(name="tween_update", type="com.sunny.game.engine.utils.easing.TweenEvent")]
	 ...
	 *
	 */
	public class STweenUtil extends EventDispatcher
	{
		STick.getInstance().addEventListener(STickEvent.TICK, tickHandler); //被引用时自动初始化
		...
		{
			new STweenUtil({}, duration, {onComplete: new SHandler(handler, para).toFunction()})
		}
		 ...
		 */
		public static function to(target : Object, duration : int, params : Object) : STweenUtil
		{
			return new STweenUtil(target, duration, params);
		}
		 ...
		 *
		 */
		public static function from(target : Object, duration : int, params : Object) : STweenUtil
		{
			params.invert = true;
			return new STweenUtil(target, duration, params);
		}
		 ...
		 *
		 */
		public static function test(target : Object, duration : int, params : Object, key : String, t : int) : STweenUtil
		{
			return new STweenUtil(target, duration, params, false);
		}
		...
		public function STweenUtil(target : Object, duration : int, params : Object, autoStart : Boolean = true)
		{
			if (!params)
				params = new Object();
			...
			this.enabledDispatchEvent = STweenUtil.enabledDispatchEvent;
			...
			for (var key : String in params)
			{
				switch (key)
				{
					...
					case "shortRotation":
						...
						this.toValues[key] = (target as DisplayObject).rotation + SMathUtil.shortRotation((target as DisplayObject).rotation, params[key], false);
					case "startAt":
						break;
					default:
						this.fromValues[key] = target[key];
		...
		{
			return STweenUtil.effects;
		}

		private static function tickHandler(event : STickEvent) : void
		{
		...
		public function update(interval : int = 0) : void
		{
			...
			else
			{
				...
				for (key in this.toValues)
				{
					t = (this.invert) ? (this.duration - this.currentTime) : this.currentTime;
					newValue = calculateValue(t, key);
		...
		public function calculateValue(t : int, key : String) : *
		{
			...
			{
				return new Rectangle(ease(t, a.x, b.x - a.x, duration), ease(t, a.y, b.y - a.y, duration), ease(t, a.width, b.width - a.width, duration), ease(t, a.height, b.height - a.height, duration))
			}
			else if (a is Array)
			{
				var newArr : Array = [];
			...
			else if (key == "tint" || key == "tint2")
			{
				...
				var g2 : int = (b >> 8) & 0xFF;
				var b2 : int = b & 0xFF;
				var r3 : int = SMathUtil.limitIn(ease(t, r1, r2 - r1, duration), 0, 0xFF);
				var g3 : int = SMathUtil.limitIn(ease(t, g1, g2 - g1, duration), 0, 0xFF);
				var b3 : int = SMathUtil.limitIn(ease(t, b1, b2 - b1, duration), 0, 0xFF);
				return SColorUtil.rgb(r3, g3, b3);
			}
		...
		protected function updateValue(key : String, value : *) : void
		{
			...
			switch (key)
			{
				...
				case "frame":
					...
					break;
				case "tint":
					displayObj.transform.colorTransform = SColorUtil.getColorTransform(value as uint);
					break;
				case "tint2":
					displayObj.transform.colorTransform = SColorUtil.getColorTransform2(value as uint);
					break;
				case "dynamicPoint":
					...
					break;
				case "motionBlur":
					var offestPoint : Point = new Point(Math.abs(value.x - displayObj.x), Math.abs(value.y - displayObj.y));
		...
		{
			var result : Array = [];
			for each (var $o : STweenUtil in effects)
			{
				if ($o.target == target)
					result.push($o);
			}
		...
		public static function removeTween(target : Object, submitEffect : Boolean = true) : void
		{
			for (var i : int = effects.length - 1; i >= 0; i--)
			{
				var $o : STweenUtil = effects[i] as STweenUtil;
				if ($o.target == target)
				{
					if (submitEffect)
						$o.duration = 0;
		...
		{
			for each (var t : STweenUtil in getTween(target))
			{
