package com.sunny.game.engine.utils.geom
{
	...
	public class Mathematic
	{
		...
		{
			var newRadian : Number = useIncrement ? Math.atan2(y - origY, x - origX) + radian : radian;
		 ...
		 * 返的对象仅供返回之后立即读取内部的值，如果打算持有返回的对象，请调用返回对象的clone方法。
		 */
		public static function rotate3D(x : Number, y : Number, z : Number, xRadianIncrement : Number, yRadianIncrement : Number, zRadianIncrement : Number) : Point3D
		{
		 ...
		 * @return
		 */
		public static function pointInTriangle(px : Number, py : Number, x1 : Number, y1 : Number, x2 : Number, y2 : Number, x3 : Number, y3 : Number) : Boolean
		{
			return !(isOnSameSide(px, py, x1, y1, x2, y2, x3, y3) || isOnSameSide(px, py, x2, y2, x3, y3, x1, y1) || isOnSameSide(px, py, x3, y3, x1, y1, x2, y2));
		}

		private static function isOnSameSide(x0 : Number, y0 : Number, x1 : Number, y1 : Number, x2 : Number, y2 : Number, x3 : Number, y3 : Number) : Boolean
		{
			var a : Number;
			var b : Number;
			var c : Number;
		 ...
		 * @return
		 */
		public static function pointInTriangleByVector(px : Number, py : Number, x1 : Number, y1 : Number, x2 : Number, y2 : Number, x3 : Number, y3 : Number, sign : Number = 1) : Boolean
		{
			_vector1.x = x2 - x1;
			_vector1.y = y2 - y1;
			_vector2.x = px - x1;
