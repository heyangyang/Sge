...
class JsonDecoder
{
	...
	private function nextToken() : int
	{
		...
		if (chr == 0x2f)
		{
			...
			else if (chr == 0x2a)
			{
				while (true)
				{
					...
					if (chr == 0x00)
						error("Find /* but cannot find */");
				}
			}
			else
				error("Unkown token /" + String.fromCharCode(chr));
			return nextToken();
		}
		lastPos = nextPos - 1;
		...
		if (isDigit(chr))
		{
			if (chr == 0x30)
			{
				...
				{
					if (!isHex(nextChar()))
						error("Need hexadecimal digit after 0x");
	...
	private function nextValue() : *
	{
			...
			if (nextToken() != 0xf9)
			{
				...
				while (true)
				{
					...
					if (nextToken() == 0xfc)
						...
						error("Unexpected token " + flush());
					if (nextToken() != 0xf7)
						error("Expected token : found " + flush());
					obj[key] = nextValue();
					if (nextToken() == 0xf9)
						break;
					if (tok != 0xf6)
		...
		if (tok == 0xfc)
			return nextString();
		if (tok == 0xfe)
			return Number(flush(1));
		if (tok == 0xff)
			error("End of input was encountered");
		if (tok != 0xf5)
			error("Unexpected token " + flush());
		return -nextValue();
	}
		...
		while (nextChar() != tag)
		{
			...
			if (chr == 0x5c)
			{
				...
				if (nextChar() == 0x75 || chr == 0x78)
				{
					...
					while (n > 0 && isHex(nextChar()))
						n--;
					if (n == 0)
						str += String.fromCharCode(parseInt(flush(), 16));
					else
						nextPos = --lastPos;
				}
				else if (chr == 0x6e)
	...
	{
		if (nextChar() != 0x2b && chr != 0x2d)
			cachedChr = true;
		if (!isDigit(nextChar()))
			error("Need digit after exponent");
	...
	private function isIdentifier(c : int) : Boolean
	{
		...
		if (c == 0xd7 || c == 0xf7)
			return false;
		if (c < 0x00c0 || c > 0xfaff)
			return false;
		if (c > 0x00d6 && c < 0x00d8)
			return false;
		if (c > 0x00f6 && c < 0x00f8)
			return false;
		if (c > 0x1fff && c < 0x3040)
			return false;
		if (c > 0x318f && c < 0x3300)
			return false;
		if (c > 0x337f && c < 0x3400)
			return false;
		if (c > 0x3d2d && c < 0x4e00)
			return false;
		if (c > 0x9fff && c < 0xf900)
			return false;
		return true;
	}
}
