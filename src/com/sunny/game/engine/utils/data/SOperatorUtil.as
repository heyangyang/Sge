package com.sunny.game.engine.utils.data
{
	...
	public class SOperatorUtil
	{
		...
		private static function getOperator(exp : String, i : int) : String
		{
			...
			{
				op = exp.slice(i, i + 5);
				if ((!mustOneTarget || isOperatorOneTarget(op)) && (op == "round" || op == "floor"))
					return op;
			}
			if (i + 3 < exp.length)
			{
				op = exp.slice(i, i + 4);
				if ((!mustOneTarget || isOperatorOneTarget(op)) && (op == "sqrt" || op == "ceil"))
					return op;
			}
			if (i + 2 < exp.length)
			{
				op = exp.slice(i, i + 3);
				if ((!mustOneTarget || isOperatorOneTarget(op)) && (op == "pow" || op == "sin" || op == "cos" || op == "tan" || op == "cot" || op == "log"))
					return op;
			}
			if (i + 1 < exp.length)
			{
				op = exp.slice(i, i + 2);
				if ((!mustOneTarget || isOperatorOneTarget(op)) && (op == "<<" || op == ">>" || op == ">=" || op == "<=" || op == "==" || op == "!=" || op == "&&" || op == "||"))
					return op;
			}
			if (i < exp.length)
			{
				op = exp.charAt(i);
				if ((!mustOneTarget || isOperatorOneTarget(op)) && (op == "+" || op == "-" || op == "*" || op == "/" || op == "%" || op == ">" || op == "<" || op == "=" || op == "&" || op == "^" || op == "|" || op == "!"))
					return op;
			}
			return null;
		}
