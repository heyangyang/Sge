...
package com.sunny.game.engine.transition.tween
{
	 ...
	 * 		  	 hex colors, volume, color, frames, and even do bezier tweening, plus LOTS more. TweenMax extends
	 * 		  	 TweenLite and adds even more capabilities like repeat, invert, repeatDelay, timeScale, event dispatching, on-the-fly
	 ...
	 * <b>PLUGINS:</b><br /><br />
	 *
	 * 	There are many plugins that add capabilities through other special properties. Some examples are "color",
	 ...
	 */
	public class STweenLite extends STweenBase
	{
		...
		public static function initClass() : void
		{
			...
			*/
			...
			rootFrame = 0;
			rootTimeline = new SSimpleTimeline(null);
			rootFramesTimeline = new SSimpleTimeline(null);
		...
		/** @private **/
		public static var killDelayedCallsTo : Function = STweenLite.clearTweens;
		/** Provides an easy way to change the default easing equation.**/
		public static var defaultEase : Function = STweenLite.easeOut;
		...
		protected static var _reservedProps : Object = {ease: 1, delay: 1, overwriteType: 1, onComplete: 1, onCompleteParams: 1, useFrames: 1, runBackwards: 1, startAt: 1, onUpdate: 1, onUpdateParams: 1, onStart: 1, onStartParams: 1, onInit: 1, onInitParams: 1, onReverseComplete: 1, onReverseCompleteParams: 1, onRepeat: 1, onRepeatParams: 1, proxiedEase: 1, easeParams: 1, invert: 1, onCompleteListener: 1, onUpdateListener: 1, onStartListener: 1, onReverseCompleteListener: 1, onRepeatListener: 1, orientToBezier: 1, timeScale: 1, immediateRender: 1, repeat: 1, repeatDelay: 1, timeline: 1, data: 1, paused: 1, reversed: 1};
		...
		public var propTweenLookup : STweenProps;
		...
		protected var _overwriteType : int;
		...
		protected var _overwrittenProps : STweenProps;
		 ...
		 */
		public function STweenLite(target : Object, duration : Number, tweenProps : STweenProps)
		{
			...
			this.target = target;
			if (this.target is STweenBase && this.tweenProps.timeScale)
			...
			propTweenLookup = new STweenProps();
			...
			_overwriteType = (!(Number(tweenProps.overwrite) > -1) || (!OverwriteManager.enabled && tweenProps.overwrite > 1)) ? OverwriteManager.mode : int(tweenProps.overwrite);
			...
			else
			{
				...
				{ //overwrite all other existing tweens of the same object (ALL mode)
					for each (var sibling : STweenLite in a)
					{
						if (!sibling.gc)
						{
			...
			if (this.active || this.tweenProps.immediateRender)
			{
		...
		{
			if (this.tweenProps.onInit)
			{
				this.tweenProps.onInit.apply(null, this.tweenProps.onInitParams);
			}
			var p : String, i : int, plugin : STweenPlugin, prioritize : Boolean, siblings : Array;
			if (typeof(this.tweenProps.ease) == "function")
			{
				_ease = this.tweenProps.ease;
			}
			if (this.tweenProps.easeParams)
			{
				this.tweenProps.proxiedEase = _ease;
				_ease = easeProxy;
			}
			this.cachedPT1 = null;
			this.propTweenLookup = new STweenProps();
			for (p in this.tweenProps.propertyList)
			{
				if (this.tweenProps[p] != null)
				{
					if (p in _reservedProps && !(p == "timeScale" && this.target is STweenBase))
					{
						//ignore
					}
					else if (p in plugins)
					{
						plugin = new (plugins[p] as Class)() as STweenPlugin;
						plugin.value = this.tweenProps[p];
						if (plugin.setup(this))
						{
							...
							if ((plugin.onDisable != null && plugin.onDisable()) || (plugin.onEnable != null && plugin.onEnable()))
							{
						...
						{
							this.cachedPT1 = new PropTween(this.target, p, Number(this.target[p]), (typeof(this.tweenProps[p]) == "number") ? Number(this.tweenProps[p]) - this.target[p] : Number(this.tweenProps[p]), p, false, this.cachedPT1);
			...
			if (this.tweenProps.runBackwards)
			{
			...
			_hasUpdate = Boolean(this.tweenProps.onUpdate != null);
			if (_overwrittenProps)
			{ //another tween may have tried to overwrite properties of this tween before init() was called (like if two tweens start at the same time, the one created second will run first)
				clearProps(_overwrittenProps);
				if (this.cachedPT1 == null)
				{ //if all tweening properties have been overwritten, kill the tween.
					this.setEnabled(false, false);
				}
			}
			if (_overwriteType > 1 && this.cachedPT1 && (siblings == masterList[this.target]) && siblings.length > 1)
			{
				if (OverwriteManager.manageOverwrites(this, this.propTweenLookup, siblings, _overwriteType))
				{
		...
		override public function renderTime(time : Number, suppressEvents : Boolean = false, force : Boolean = false) : void
		{
			...
			if (prevTime == 0 && this.tweenProps.onStart && (this.cachedTime != 0 || this.cachedDuration == 0) && !suppressEvents)
			{
		 ...
		 */
		public function clearProps(tweenProps : STweenProps, permanent : Boolean = true) : Boolean
		{
			if (_overwrittenProps == null)
			{
				_overwrittenProps = new STweenProps();
			}
			var p : String, pt : PropTween, changed : Boolean;
			for (p in tweenProps.propertyList)
			{
				if (p in propTweenLookup.propertyList)
				{
					pt = propTweenLookup[p];
					if (pt.isPlugin && pt.name == "_MULTIPLE_")
					{
						pt.target.killProps(tweenProps);
				...
				if (permanent && tweenProps != _overwrittenProps)
				{
		...
		override public function setEnabled(enabled : Boolean, ignoreTimeline : Boolean = false) : Boolean
		{
			if (enabled)
			{
				var a : Array = STweenLite.masterList[this.target];
				if (!a)
				{
					STweenLite.masterList[this.target] = [this];
				}
...
//---- STATIC FUNCTIONS -----------------------------------------------------------------------------------
		 ...
		 */
		public static function to(target : Object, duration : Number, tweenProps : STweenProps) : STweenLite
		{
			return new STweenLite(target, duration, tweenProps);
		}
		 ...
		 * @return TweenLite instance
		 */
		public static function from(target : Object, duration : Number, tweenProps : STweenProps) : STweenLite
		{
		 ...
		 * @return TweenLite instance
		 */
		public static function delayedCall(delay : Number, onComplete : Function, onCompleteParams : Array = null, useFrames : Boolean = false) : STweenLite
		{
			...
			return new STweenLite(onComplete, 0, tweenProps);
		}
		 ...
		 * @param complete Indicates whether or not the tweens should be forced to completion before being killed.
		 */
		public static function clearTweens(target : Object, tweenProps : STweenProps = null, complete : Boolean = false) : void
		{
			if (target in masterList)
			{
				...
				while (--i > -1)
				{
					...
					if (!tween.gc)
					{
						...
						if (tweenProps != null)
						{
							tween.clearProps(tweenProps);
						}
						if (tweenProps == null || (tween.cachedPT1 == null && tween.initted))
						{
		...
		{
			return this.tweenProps.proxiedEase.apply(null, arguments.concat(this.tweenProps.easeParams));
		}
