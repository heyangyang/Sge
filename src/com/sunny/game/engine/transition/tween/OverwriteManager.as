...
package com.sunny.game.engine.transition.tween
{
	 ...
	 *
	 * 		TweenMax.to(mc, 1, {x:100, overwriteType:OverwriteManager.PREXISTING});<br />
	 * 		TweenMax.to(mc, 1, {x:100, overwriteType:5});<br /><br /></code>
	 *
	...
	final public class OverwriteManager
	{
		 ...
		 **/
		public static function manageOverwrites(tween : STweenLite, props : STweenProps, targetTweens : Array, mode : int) : Boolean
		{
			var i : int, changed : Boolean, curTween : STweenLite;
			if (mode >= 4)
			{
			...
			{ //tweens that are nested in other timelines may have various offsets and timeScales so we need to translate them to a global/root one to see how they compare.
				var combinedTimeScale : Number = tween.cachedTimeScale, combinedStartTime : Number = startTime, cousin : STweenBase, cousinStartTime : Number, timeline : SSimpleTimeline;
			...
			else
			{
				while (--i > -1)
				{
					if (STweenLite(overlaps[i]).setEnabled(false, false))
		...
		/** @private **/
		public static function getGlobalPaused(tween : STweenBase) : Boolean
		{
			var paused : Boolean;
			while (tween)
			{
