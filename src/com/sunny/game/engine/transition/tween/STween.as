package com.sunny.game.engine.transition.tween
{
	...
	import com.sunny.game.engine.transition.tween.core.SSimpleTimeline;
	import com.sunny.game.engine.transition.tween.core.STweenBase;
	import com.sunny.game.engine.transition.tween.events.TweenEvent;
	import com.sunny.game.engine.transition.tween.plugins.RemoveTintPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SAutoAlphaPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SBevelFilterPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SBezierPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SBlurFilterPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SColorMatrixFilterPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SColorTransformPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SDropShadowFilterPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SEndArrayPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SFrameLabelPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SFramePlugin;
	import com.sunny.game.engine.transition.tween.plugins.SGlowFilterPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SHexColorsPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SRoundPropsPlugin;
	import com.sunny.game.engine.transition.tween.plugins.SShortRotationPlugin;
	...
	import com.sunny.game.engine.transition.tween.plugins.SVisiblePlugin;
	import com.sunny.game.engine.transition.tween.plugins.SVolumePlugin;
	 ...
	 * 	like timeScale, event dispatching, updateTo(), invert, repeat, repeatDelay, rounding, and more. It also
	 ...
	 * 		  	hex colors, volume, color, frames, saturation, contrast, hue, colorization, brightness, and even do
	 ...
	 *
	 * 	<li><b> invert : Boolean</b> 					Works in conjunction with the repeat property, determining the behavior of each
	 * 												cycle. When invert is true, the tween will go back and forth, appearing to reverse
	 * 												every other cycle (this has no affect on the "reversed" property though). So if repeat is
	 * 												2 and invert is false, it will look like: start - 1 - 2 - 3 - 1 - 2 - 3 - 1 - 2 - 3 - end. But
	 * 												if repeat is 2 and invert is true, it will look like: start - 1 - 2 - 3 - 3 - 2 - 1 - 1 - 2 - 3 - end. </li>
	 *
	 ...
	 *
	 * 	  <li><b> color : Number</b> - To change a DisplayObject's color/color, set this to the hex value of the color you'd like
	 * 					  			  to end up at(or begin at if you're using TweenMax.from()). An example hex value would be 0xFF0000.</li>
	 *
	 * 	  <li><b> removeColor : Boolean</b> - If you'd like to remove the color that's applied to a DisplayObject, pass true for this special property.</li>
	 *
	 ...
	 *
	 */
	public class STween extends STweenLite implements IEventDispatcher
	{
		/** @private **/
		public static const version : Number = 11.698;

		STweenPlugin.activate([


			//ACTIVATE (OR DEACTIVATE) PLUGINS HERE...

			SAutoAlphaPlugin, //tweens alpha and then toggles "visible" to false if/when alpha is zero
			SEndArrayPlugin, //tweens numbers in an Array
			SFramePlugin, //tweens MovieClip frames
			RemoveTintPlugin, //allows you to remove a color
			STintPlugin, //tweens tints
			SVisiblePlugin, //tweens a target's "visible" property
			SVolumePlugin, //tweens the volume of a MovieClip or SoundChannel or anything with a "soundTransform" property

			SBevelFilterPlugin, //tweens BevelFilters
			SBezierPlugin, //enables bezier tweening
			SBlurFilterPlugin, //tweens BlurFilters
			SColorMatrixFilterPlugin, //tweens ColorMatrixFilters (including hue, saturation, colorize, contrast, brightness, and threshold)
			SColorTransformPlugin, //tweens advanced color properties like exposure, brightness, tintAmount, redOffset, redMultiplier, etc.
			SDropShadowFilterPlugin, //tweens DropShadowFilters
			SFrameLabelPlugin, //tweens a MovieClip to particular label
			SGlowFilterPlugin, //tweens GlowFilters
			SHexColorsPlugin, //tweens hex colors
			SRoundPropsPlugin, //enables the roundProps special property for rounding values (ONLY for TweenMax!)
			SShortRotationPlugin, //tweens rotation values in the shortest direction
		...
		/**
		 * Works in conjunction with the repeat property, determining the behavior of each cycle; when invert is true,
		 * the tween will go back and forth, appearing to reverse every other cycle (this has no affect on the "reversed"
		 * property though). So if repeat is 2 and invert is false, it will look like: start - 1 - 2 - 3 - 1 - 2 - 3 - 1 - 2 - 3 - end.
		 * But if repeat is 2 and invert is true, it will look like: start - 1 - 2 - 3 - 3 - 2 - 1 - 1 - 2 - 3 - end.
		 **/
		 ...
		 */
		public function STween(target : Object, duration : Number, tweenProps : STweenProps)
		{
			super(target, duration, tweenProps);
			if (STweenLite.version < 11.2)
			{
				throw new Error("TweenMax error! Please update your TweenLite class or try deleting your ASO files. TweenMax requires a more recent version. Download updates at http://www.TweenMax.com.");
			}
			this.invert = Boolean(this.tweenProps.invert);
			_repeat = uint(this.tweenProps.repeat);
			_repeatDelay = (this.tweenProps.repeatDelay) ? Number(this.tweenProps.repeatDelay) / 1000 : 0;
			...
			if (this.tweenProps.onCompleteListener || this.tweenProps.onInitListener || this.tweenProps.onUpdateListener || this.tweenProps.onStartListener || this.tweenProps.onRepeatListener || this.tweenProps.onReverseCompleteListener)
			{
				initDispatcher();
			...
			if (this.tweenProps.timeScale && !(this.target is STweenBase))
			{
				this.cachedTimeScale = this.tweenProps.timeScale;
			}
		}
		...
		{
			if (this.tweenProps.startAt)
			{
				this.tweenProps.startAt.overwriteType = 0;
				this.tweenProps.startAt.immediateRender = true;
				var startTween : STween = new STween(this.target, 0, this.tweenProps.startAt);
			}
			if (_dispatcher)
			{
		...
		{
			this.invert = Boolean(this.tweenProps.invert == true);
			_repeat = (this.tweenProps.repeat) ? Number(this.tweenProps.repeat) : 0;
			_repeatDelay = (this.tweenProps.repeatDelay) ? Number(this.tweenProps.repeatDelay) : 0;
			_hasUpdateListener = false;
			if (this.tweenProps.onCompleteListener != null || this.tweenProps.onUpdateListener != null || this.tweenProps.onStartListener != null)
			{
				initDispatcher();
			}
			setDirtyCache(true);
		...
		{
			var v : STweenProps = new STweenProps(), i : int = names.length;
			while (--i > -1)
			{
				v[names[i]] = true;
			}
		...
		override public function renderTime(time : Number, suppressEvents : Boolean = false, force : Boolean = false) : void
		{
			...
			if (_repeat != 0)
			{
				...
				else if (time > 0)
				{
					...
					if (this.invert && _cyclesComplete % 2)
					{
			...
			{
				if (this.tweenProps.onStart != null)
				{
					this.tweenProps.onStart.apply(null, this.tweenProps.onStartParams);
				}
				if (_dispatcher)
				{
			...
			{
				this.tweenProps.onUpdate.apply(null, this.tweenProps.onUpdateParams);
			}
			...
			{
				if (this.tweenProps.onRepeat)
				{
					this.tweenProps.onRepeat.apply(null, this.tweenProps.onRepeatParams);
				}
				if (_dispatcher)
				{
...
//---- EVENT DISPATCHING ----------------------------------------------------------------------------------------------------------
		...
		protected function initDispatcher() : void
		{
			...
			if (this.tweenProps.onInitListener is Function)
			{
				_dispatcher.addEventListener(TweenEvent.INIT, this.tweenProps.onInitListener, false, 0, true);
			}
			if (this.tweenProps.onStartListener is Function)
			{
				_dispatcher.addEventListener(TweenEvent.START, this.tweenProps.onStartListener, false, 0, true);
			}
			if (this.tweenProps.onUpdateListener is Function)
			{
				_dispatcher.addEventListener(TweenEvent.UPDATE, this.tweenProps.onUpdateListener, false, 0, true);
				_hasUpdateListener = true;
			}
			if (this.tweenProps.onCompleteListener is Function)
			{
				_dispatcher.addEventListener(TweenEvent.COMPLETE, this.tweenProps.onCompleteListener, false, 0, true);
			}
			if (this.tweenProps.onRepeatListener is Function)
			{
				_dispatcher.addEventListener(TweenEvent.REPEAT, this.tweenProps.onRepeatListener, false, 0, true);
			}
			if (this.tweenProps.onReverseCompleteListener is Function)
			{
				_dispatcher.addEventListener(TweenEvent.REVERSE_COMPLETE, this.tweenProps.onReverseCompleteListener, false, 0, true);
			}
		}
...
//---- STATIC FUNCTIONS -----------------------------------------------------------------------------------------------------------
		 ...
		 *
		 * @param target Target object whose properties this tween affects. This can be ANY object, not just a DisplayObject. 作为目标的对象， MovieClip或者其它对象
		 * @param duration Duration in seconds (or in frames for frames-based tweens) 动画的时间长度（单位：秒）
		 * @param vars An object containing the end values of the properties you're tweening. For example, to tween to x=100, y=100, you could pass {x:100, y:100}. It can also contain special properties like "onComplete", "ease", "delay", etc. 对象，通过属性值，来存贮各种属性参数用于缓动。
		 * @return TweenMax instance
		 */
		public static function to(target : Object, duration : Number, tweenProps : STweenProps) : STween
		{
			return new STween(target, duration, tweenProps);
		}

		public static function clear(target : Object, tweenProps : STweenProps = null) : void
		{
			STweenLite.clearTweens(target, tweenProps, false);
		}
		 ...
		 * @param vars An object containing the start values of the properties you're tweening. For example, to tween from x=100, y=100, you could pass {x:100, y:100}. It can also contain special properties like "onComplete", "ease", "delay", etc. （如果你使用TweenLite.from() 方法，这里的参数表示缓动的初始值）
		 * @return TweenMax instance
		 */
		public static function from(target : Object, duration : Number, tweenProps : STweenProps) : STween
		{
			if (tweenProps.isGSVars)
			{ //to accommodate TweenMaxVars instances for strong data typing and code hinting
				tweenProps = tweenProps.vars;
			}
			tweenProps.runBackwards = true;
			if (!("immediateRender" in tweenProps))
			{
				tweenProps.immediateRender = true;
			}
			return new STween(target, duration, tweenProps);
		}
		 ...
		 * @return TweenMax instance
		 */
		public static function fromTo(target : Object, duration : Number, fromTweenProps : STweenProps, toTweenProps : STweenProps) : STween
		{
			if (toTweenProps.isGSVars)
			{ //to accommodate TweenMaxVars instances for strong data typing and code hinting
				toTweenProps = toTweenProps.vars;
			}
			if (fromTweenProps.isGSVars)
			{ //to accommodate TweenMaxVars instances for strong data typing and code hinting
				fromTweenProps = fromTweenProps.vars;
			}
			toTweenProps.startAt = fromTweenProps;
			if (fromTweenProps.immediateRender)
			{
				toTweenProps.immediateRender = true;
			}
			return new STween(target, duration, toTweenProps);
		}
		 ...
		 */
		public static function groupTo(targets : Array, duration : Number, tweenProps : STweenProps, stagger : Number = 0, onCompleteAll : Function = null, onCompleteParams : Array = null) : Array
		{
			var i : int, tweenPropsDup : STweenProps, p : String;
			...
			var curDelay : Number = ("delay" in tweenProps) ? Number(tweenProps.delay) : 0;
			var onCompleteProxy : Function = tweenProps.onComplete;
			var onCompleteParamsProxy : Array = tweenProps.onCompleteParams;
			var lastIndex : int = l - 1;
			for (i = 0; i < l; i += 1)
			{
				tweenPropsDup = tweenProps.clone();
				tweenPropsDup.delay = curDelay;
				if (i == lastIndex && onCompleteAll != null)
				{
					tweenPropsDup.onComplete = function() : void
					{
						if (onCompleteProxy != null)
						{
							onCompleteProxy.apply(null, onCompleteParamsProxy);
						}
						onCompleteAll.apply(null, onCompleteParams);
					}
				}
				a[i] = new STween(targets[i], duration, tweenPropsDup);
				curDelay += stagger;
			}
			return a;
		}
		 ...
		 */
		public static function groupFrom(targets : Array, duration : Number, tweenProps : STweenProps, stagger : Number = 0, onCompleteAll : Function = null, onCompleteParams : Array = null) : Array
		{
			if (tweenProps.isGSVars)
			{ //to accommodate TweenMaxVars instances for strong data typing and code hinting
				tweenProps = tweenProps.vars;
			...
			tweenProps.runBackwards = true;
			if (!("immediateRender" in tweenProps)) //属性可能错了
			{
				tweenProps.immediateRender = true;
			}
			return groupTo(targets, duration, tweenProps, stagger, onCompleteAll, onCompleteParams);
		}
		 ...
		 * @return Array of TweenMax instances
		 */
		public static function groupFromTo(targets : Array, duration : Number, fromTweenProps : STweenProps, toTweenProps : STweenProps, stagger : Number = 0, onComplete : Function = null, onCompleteParams : Array = null) : Array
		{
			if (toTweenProps.isGSVars)
			{ //to accommodate TweenMaxVars instances for strong data typing and code hinting
				toTweenProps = toTweenProps.vars;
			...
			return groupTo(targets, duration, toTweenProps, stagger, onComplete, onCompleteParams);
		}
		 ...
		 * @return TweenMax instance
		 */
		public static function delayedCall(delay : Number, onComplete : Function, onCompleteParams : Array = null, useFrames : Boolean = false) : STween
		{
			var tweenProps : STweenProps = new STweenProps();
			tweenProps.delay = delay;
			tweenProps.onComplete = onComplete;
			tweenProps.onCompleteParams = onCompleteParams;
			tweenProps.immediateRender = false;
			tweenProps.useFrames = useFrames;
			tweenProps.overwriteType = 0;
			return new STween(onComplete, 0, tweenProps);
		}
		...
		private static function changePause(pause : Boolean, tweens : Boolean = true, delayedCalls : Boolean = false) : void
		{
			...
			while (--i > -1)
			{
				isDC = (STweenLite(a[i]).target == STweenLite(a[i]).tweenProps.onComplete);
				if (isDC == delayedCalls || isDC != tweens)
				{
					STweenBase(a[i]).paused = pause;
				}
			}
		}
	 ...
	 * where 0 is at the beginning, 0.5 is halfway finished, and 1 is finished. <code>currentProgress</code>,
		...
		* once, at the end of the first cycle <code>totalProgress</code> would only be 0.5
		...
		override public function set currentTime(n : Number) : void
		{
			...
			else if (this.invert && (_cyclesComplete % 2 == 1))
			{
		...
		{
			return (STweenLite.rootTimeline == null) ? 1 : STweenLite.rootTimeline.cachedTimeScale;
		}
		...
		public static function set globalTimeScale(n : Number) : void
		{
			...
			if (STweenLite.rootTimeline == null)
			{
				STweenLite.to({}, 0, new STweenProps()); //forces initialization in case globalTimeScale is set before any tweens are created.
			}
			var tl : SSimpleTimeline = STweenLite.rootTimeline;
			var curTime : Number = (getTimer() * 0.001)
			tl.cachedStartTime = curTime - ((curTime - tl.cachedStartTime) * tl.cachedTimeScale / n);
			tl = STweenLite.rootFramesTimeline;
			curTime = STweenLite.rootFrame;
			tl.cachedStartTime = curTime - ((curTime - tl.cachedStartTime) * tl.cachedTimeScale / n);
			STweenLite.rootFramesTimeline.cachedTimeScale = STweenLite.rootTimeline.cachedTimeScale = n;
		}
	}
}