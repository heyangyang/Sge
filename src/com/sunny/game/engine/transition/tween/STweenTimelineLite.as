package com.sunny.game.engine.transition.tween
{
	 ...
	 *
	 * 		<li> If you need even more features like AS3 event listeners, <code>repeat, repeatDelay, invert, currentLabel,
	 ...
	 *
	 * 		var myTimeline:TimelineLite = new TimelineLite({tweens:[new TweenLite(mc1, 1, {y:"100"}), TweenMax.to(mc2, 1, {color:0xFF0000})], align:TweenAlign.SEQUENCE, onComplete:myFunction});</code><br /><br />
	 *
	 ...
	 **/
	public class STweenTimelineLite extends SSimpleTimeline
	{
		/** @private **/
		 ...
		 */
		public function STweenTimelineLite(tweenProps : STweenProps = null)
		{
			super(tweenProps);
			_endCaps = [null, null];
			_labels = {};
			this.autoRemoveChildren = Boolean(this.tweenProps.autoRemoveChildren == true);
			_hasUpdate = Boolean(typeof(this.tweenProps.onUpdate) == "function");
			if (this.tweenProps.tweens is Array)
			{
				this.insertMultiple(this.tweenProps.tweens, 0, (this.tweenProps.align != null) ? this.tweenProps.align : "normal", (this.tweenProps.stagger) ? Number(this.tweenProps.stagger) : 0);
			}
		}
		 ...
		 */
		override public function remove(tween : STweenBase, skipDisable : Boolean = false) : void
		{
			...
			var first : STweenBase = (this.gc) ? _endCaps[0] : _firstChild;
			var last : STweenBase = (this.gc) ? _endCaps[1] : _lastChild;
		 ...
		 */
		override public function insert(tween : STweenBase, timeOrLabel : * = 0) : STweenBase
		{
			...
			{
				var curTween : STweenBase = last, st : Number = tween.cachedStartTime;
			...
			{
				if (this.timeline == STweenLite.rootTimeline || this.timeline == STweenLite.rootFramesTimeline)
				...
				var tl : SSimpleTimeline = this.timeline;
		 ...
		 */
		public function append(tween : STweenBase, offset : Number = 0) : STweenBase
		{
		 ...
		 */
		public function prepend(tween : STweenBase, adjustLabels : Boolean = false) : STweenBase
		{
		...
		{
			var i : int, tween : STweenBase, curTime : Number = Number(timeOrLabel) || 0, l : int = tweens.length;
			if (typeof(timeOrLabel) == "string")
			{
				if (!(timeOrLabel in _labels))
				{
		...
		{
			var tweenProps : STweenProps = new STweenProps({tweens: tweens, align: align, stagger: stagger});
			var tl : STweenTimelineLite = new STweenTimelineLite(tweenProps); //dump them into a new temporary timeline initially so that we can determine the overall duration.
		...
		override public function renderTime(time : Number, suppressEvents : Boolean = false, force : Boolean = false) : void
		{
			...
			if (prevTime == 0 && this.tweenProps.onStart && this.cachedTime != 0 && !suppressEvents)
			{
		...
		{
			var tween : STweenBase = _lastChild, next : STweenBase, dur : Number;
		...
		{
			var tween : STweenBase = _firstChild, next : STweenBase, dur : Number;
			var prevPaused : Boolean = this.cachedPaused;
			while (tween)
			{
				next = tween.nextNode; //record it here because the value could change after rendering...
		...
		{
			var tween : STweenBase = (this.gc) ? _endCaps[0] : _firstChild;
			while (tween)
			{
				if (tween.cachedPaused || ((tween is STweenTimelineLite) && (tween as STweenTimelineLite).hasPausedChild()))
				{
					return true;
				}
		...
		{
			var a : Array = [], cnt : int = 0, tween : STweenBase = (this.gc) ? _endCaps[0] : _firstChild;
			while (tween)
			{
				...
				else
				{
					...
					if (nested)
					{
						a = a.concat(STweenTimelineLite(tween).getChildren(true, tweens, timelines));
						cnt = a.length;
					}
				}
				tween = tween.nextNode;
		...
		public function getTweensOf(target : Object, nested : Boolean = true) : Array
		{
			...
			for (i = 0; i < l; i += 1)
			{
				if (STweenLite(tweens[i]).target == target)
				{
					a[cnt++] = tweens[i];
				}
			}
		 ...
		 */
		public function killTweensOf(target : Object, nested : Boolean = true, tweenProps : STweenProps = null) : Boolean
		{
		...
		public function clear(tweens : Array = null) : void
		{
			...
			while (--i > -1)
			{
				STweenBase(tweens[i]).setEnabled(false, false);
			}
		}
...
//---- GETTERS / SETTERS -------------------------------------------------------------------------------------------------------
	 ...
	 * but in TimelineMax, they could be different. For example, if a TimelineMax instance
		...
		override public function get totalDuration() : Number
		{
			if (this.cacheIsDirty)
			{
				var max : Number = 0, end : Number, tween : STweenBase = (this.gc) ? _endCaps[0] : _firstChild, prevStart : Number = -Infinity, next : STweenBase;
				while (tween)
				{
		...
		public function get useFrames() : Boolean
		{
			...
			return Boolean(tl == STweenLite.rootFramesTimeline);
		}
