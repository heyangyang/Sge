package com.sunny.game.engine.transition.tween
{
	 ...
	 *  (but non-essential) features like AS3 event dispatching, repeat, repeatDelay, invert,
	 ...
	 * 		  between each repeat cycle and/or cause the repeat cycles to invert, appearing to reverse
	 ...
	 * 		myTimeline.prepend(new TweenMax(mc, 1, {color:0xFF0000}));<br /><br />
	 *
	 ...
	 *
	 * 		var myTimeline:TimelineMax = new TimelineMax({tweens:[new TweenLite(mc1, 1, {y:"100"}), TweenMax.to(mc2, 1, {color:0xFF0000})], align:TweenAlign.SEQUENCE, onComplete:myFunction, repeat:2, repeatDelay:1});</code><br /><br />
	 *
	 ...
	 **/
	public class STweenTimeline extends STweenTimelineLite implements IEventDispatcher
	{
		/** @private **/
		...
		/**
		 * Works in conjunction with the repeat property, determining the behavior of each cycle; when <code>invert</code> is true,
		 ...
		 * So if repeat is 2 and <code>invert</code> is false, it will look like: start - 1 - 2 - 3 - 1 - 2 - 3 - 1 - 2 - 3 - end.
		 * But if repeat is 2 and <code>invert</code> is true, it will look like: start - 1 - 2 - 3 - 3 - 2 - 1 - 1 - 2 - 3 - end.
		 **/
		 ...
		 * <code>new TimelineMax({paused:true, onComplete:myFunction, repeat:2, invert:true})</code>
		 *
		 * <ul>
		 ...
		 * 											cycle. When <code>invert</code> is true, the timeline will go back and forth, appearing to reverse
		 ...
		 * 	</ul>
		 *
		 * @param vars optionally pass in special properties like useFrames, onComplete, onCompleteParams, onUpdate, onUpdateParams, onStart, onStartParams, tweens, align, stagger, delay, autoRemoveChildren, onCompleteListener, onStartListener, onUpdateListener, repeat, repeatDelay, and/or invert.
		 */
		public function STweenTimeline(tweenProps : STweenProps = null)
		{
			super(tweenProps);
			...
			if (this.tweenProps.onCompleteListener != null || this.tweenProps.onUpdateListener != null || this.tweenProps.onStartListener != null || this.tweenProps.onRepeatListener != null || this.tweenProps.onReverseCompleteListener != null)
			{
				initDispatcher();
			}
		}
		 ...
		 */
		public function addCallback(callback : Function, timeOrLabel : *, params : Array = null) : STweenLite
		{
			var tweenProps : STweenProps = new STweenProps({onComplete: callback, onCompleteParams: params, overwriteType: 0, immediateRender: false});
			var cb : STweenLite = new STweenLite(callback, 0, tweenProps);
		 ...
		 */
		public function tweenTo(timeOrLabel : *, vars : Object = null) : STweenLite
		{
			var varsCopy : Object = {ease: easeNone, overwriteType: 2, useFrames: this.useFrames, immediateRender: false};
			...
			var tl : STweenLite = new STweenLite(this, (Math.abs(Number(varsCopy.currentTime) - this.cachedTime) / this.cachedTimeScale) || 0.001, new STweenProps(varsCopy));
			tl.tweenProps.onInitParams[0] = tl;
			return tl;
		}
		 ...
		 */
		public function tweenFromTo(fromTimeOrLabel : *, toTimeOrLabel : *, vars : Object = null) : STweenLite
		{
			var tl : STweenLite = tweenTo(toTimeOrLabel, vars);
			tl.tweenProps.onInitParams[2] = parseTimeOrLabel(fromTimeOrLabel);
			tl.duration = Math.abs(Number(tl.tweenProps.currentTime) - tl.tweenProps.onInitParams[2]) / this.cachedTimeScale;
			return tl;
		}

		/** @private **/
		private static function onInitTweenTo(tween : STweenLite, timeline : STweenTimeline, fromTime : Number) : void
		{
			...
			if (tween.tweenProps.currentTime != timeline.currentTime)
			{ //don't make the duration zero - if it's supposed to be zero, don't worry because it's already initting the tween and will complete immediately, effectively making the duration zero anyway. If we make duration zero, the tween won't run at all.
				tween.duration = Math.abs(Number(tween.tweenProps.currentTime) - timeline.currentTime) / timeline.cachedTimeScale;
			}
		}
			...
			if (_repeat != 0)
			{
				...
				if (repeated && !isComplete && (this.cachedTotalTime != prevTotalTime || force))
				{
					...
					/*
					  ...
					  would get transated to 2.8 seconds if the timeline inverts or 0.2 seconds if it just repeats), there
					  could be a callback or a short tween that's at 2.95 or 3 seconds in which wouldn't render. So
					  we need to push the timeline to the end (and/or beginning depending on its invert value).
					*/

					var forward : Boolean = Boolean(!this.invert || (_cyclesComplete % 2 == 0));
					var prevForward : Boolean = Boolean(!this.invert || (prevCycles % 2 == 0));
		...
		{
			var a : Array = [], all : Array = getChildren(nested, tweens, timelines), i : int, tween : STweenBase;
