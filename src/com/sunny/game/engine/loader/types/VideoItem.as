package com.sunny.game.engine.loader.types
{
	...
	public class VideoItem extends LoadingItem
	{
		...
		{
			specificAvailableProps = [SResourceLoader.CHECK_POLICY_FILE, SResourceLoader.PAUSED_AT_START];
			super(url, type, uid);
		...
		{
			pausedAtStart = props[SResourceLoader.PAUSED_AT_START] || false;
			_checkPolicyFile = props[SResourceLoader.CHECK_POLICY_FILE] || false;
		...
		private function fireCanBeginStreamingEvent() : void
		{
			...
			var evt : Event = new Event(SResourceLoader.CAN_BEGIN_PLAYING);
			dispatchEvent(evt);
		}
