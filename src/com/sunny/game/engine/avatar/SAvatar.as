package com.sunny.game.engine.avatar
{
	...
	public class SAvatar
	{
		...
		public var render : SBaseRenderData;
		public var mouseRect : Rectangle = new Rectangle();

		public var loaderComplement : Function;
		public var changeAnimation : Function;

		public static var bigShadowData : SIBitmapData;
		public static var smallShadowData : SIBitmapData;
		public static var midShadowData : SIBitmapData;
		...
		public function SAvatar(desc : SAvatarDescription)
		{
			...
			render = new SBaseRenderData.renderClass();
			render.name = desc.name;
			render.scaleX = this.scaleX;
			render.scaleY = this.scaleY;
		}
		...
		public function gotoAnimation(action : uint, kind : uint, dir : int, frame : int, loops : int) : SAnimationFrame
		{
			if (avatarDesc == null)
			{
				SDebug.warningPrint(this, "avatarDesc=null");
				return null;
			}
			if (action != 0)
			{
				...
				else
				{
					...
					if (SDebug.OPEN_INFO_TRACE)
						SDebug.infoPrint(this, avatarDesc.name + "配置文件不存在动作" + action + "的资源，将取有效动作：" + _curAction);
				}
			}
			...
			{
				if (SDebug.OPEN_INFO_TRACE)
					SDebug.infoPrint(this, avatarDesc.name + "配置文件错误，没有一个可用的动作的资源：动作-" + action + '方向-' + dir);
				return null;
			}
			...
			if (_correctDir != 0)
			{
				if (!hasDir(_correctDir, _curAction, _curKind))
				{
					...
					if (SDebug.OPEN_INFO_TRACE)
						SDebug.infoPrint(this, avatarDesc.name + "配置文件动作" + action + "不存在方向" + dir + "的资源，将取有效方向：" + _correctDir);
				}
			}
...
//			}
			...
			if (_curAnimationFrame.frameData && render.bitmapData != _curAnimationFrame.frameData)
			{
				...
				{
					mouseRect.left = _curAnimationFrame.x * scaleX - (_curAnimationFrame.needReversal ? mouseRect.width : 0);
					mouseRect.top = _curAnimationFrame.y * scaleX;
					mouseRect.width = _curAnimationFrame.rect.width * scaleX;
					mouseRect.height = _curAnimationFrame.rect.height * scaleY;
				}
			}
		}
		...
		public function updateRender(bufferX : Number, bufferY : Number) : void
		{
			...
			{
				render.x = bufferX + _updateAnimationFrame.x * scaleX;
				render.y = bufferY + _updateAnimationFrame.y * scaleY;
			}
		}
		...
		public function get currRender() : SBaseRenderData
		{
			return render;
		}
		...
		public function set animationsByParts(value : SAvatarAnimationLibrary) : void
		{
			...
			render.isLockColorTransform = true;
		...
		private function onLoadCompleteAnimation() : void
		{
			...
			render.isLockColorTransform = false;
			//加载完成，实行一次
		...
		public function isRolePickable(localX : int, localY : int) : Boolean
		{
			if (curAnimationFrame && curAnimationFrame.frameData)
			{
				...
				if (curAnimationFrame.needReversal)
					localX = curAnimationFrame.frameData.width * scaleX - localX;

				var color : uint = curAnimationFrame.frameData.getPixel(localX / scaleX, localY / scaleY);
